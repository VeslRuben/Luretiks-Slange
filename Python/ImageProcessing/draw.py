...
def drawSection(image, center, startangle, endAngle, color, radius=50):
    """
    ...
    image = cv2.ellipse(image, center, (radius, radius), 0, startangle, endAngle, color, 2)
    ...
    image = cv2.line(image, center, (x1, y1), color, 2)
    image = cv2.line(image, center, (x2, y2), color, 2)
    return image
...
def drawCollisionSectors(image, snakeCoordinates, angleList, collisions, offset, radius=50):
    ...
    if offset < 0:
        offset += 360
    for pos, piece, collision in zip(snakeCoordinates, angleList, collisions):
        i = 0
